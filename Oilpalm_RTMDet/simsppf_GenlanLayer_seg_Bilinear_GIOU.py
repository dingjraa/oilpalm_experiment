...
default_hooks = dict(
    checkpoint=dict(interval=1, max_keep_ckpts=1000, type='CheckpointHook'),
    logger=dict(interval=1000, type='LoggerHook'),
    param_scheduler=dict(type='ParamSchedulerHook'),
    sampler_seed=dict(type='DistSamplerSeedHook'),
    timer=dict(type='IterTimerHook'),
    visualization=dict(
        draw=False, show=False, type='DetVisualizationHook', wait_time=2))
default_scope = 'mmdet'
env_cfg = dict(
...
interval = 10
launcher = 'none'
load_from = '.\\work_dirs\\rtmdet\\fulldata\\test\\three\\simsppf_GenlanLayer_seg_Bilinear_GIOU\\epoch_91.pth'
log_level = 'INFO'
...
model = dict(
    ...
    neck=dict(
        ...
        num_csp_blocks=1,
        out_channels=128,
        type='GELAN_UpsamplingBilinear_ca_PAFPN'),
    test_cfg=dict(
        max_per_img=300,
        min_bbox_size=0,
...
test_evaluator = dict(
    ...
    format_only=True,
    metric='bbox',
    outfile_prefix='./outfile_prefix/sim_elan_seg_bilinear',
    proposal_nums=(
        100,
        1,
        10,
...
work_dir = './work_dirs\\rtmdet/fulldata/test/three/simsppf_GenlanLayer_seg_Bilinear_GIOU'
